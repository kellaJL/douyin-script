...
def save_debug_screenshot_font(ts,age,beauty):
    """
    对 debug 图片加上详细的注释
    
    """
    im=Image.open('optimized.png')
    make_debug_dir(screenshot_backup_dir)
    draw = ImageDraw.Draw(im)
    ttfont = ImageFont.truetype("C:\\WINDOWS\\Fonts\\simhei.TTF",72)  
    draw = ImageDraw.Draw(im)
    draw.text((10,10),str(age), fill=(255,0,0),font=ttfont)
    draw.text((10,100),str(beauty), fill=(255,0,0),font=ttfont)
    im.save(os.path.join(os.getcwd(), screenshot_backup_dir,
                         '#' + str(ts) + '.png'))
   

    with open(os.path.join(os.getcwd(), screenshot_backup_dir,'ts.txt'),'w') as f:
        f.write(str(ts))
    f.close()
